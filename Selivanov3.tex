...
c_\alpha=F_\alpha(1),
\qquad
F_\alpha(\xi)=\int_0^\xi t_\alpha(s)\,\dd s .
\]
Here \(t_\alpha(\xi)\) is the normalized traction--separation law in pure
mode \(\alpha\), \(F_\alpha(\xi)\) is the associated dimensionless
potential, and \(c_\alpha\) is the corresponding dimensionless area under
the normalized traction curve. It is convenient to introduce the energetic
prefactors
\[
\chi_\alpha:=\sigma_\alpha^{\max}d_\alpha^{\max}
...
For notational brevity, once the pure-mode laws have been specified, we
write \(t(\cdot)\) and \(F(\cdot)\) for the corresponding mode-dependent
functions, with the active mode understood from the argument and the
associated parameter set.

\paragraph{Mode-wise (one-dimensional) law.}
The purpose of the strengthening branch has already been discussed in
...
\(0<a_1<a_2<1\) define the end of the strengthening interval and the onset
of softening, respectively. In applications, these parameters may be chosen
separately for the normal and tangential modes; the mode index is omitted
here for compactness.

For the normal mode, compression is regularized by a linear penalty branch.
The normalized traction \(t\), its derivative \(g=t'\), and the associated
potential \(F\) are written in the compact form
\[
(F,t,g)(\xi)=
\begin{cases}
	\left(\dfrac{\kappa}{2}\xi^2,\ \kappa\xi,\ \kappa\right),
	& \xi\le 0,\\[8pt]
	\left(
	\dfrac{\xi^2}{a_1}\Bigl(1-\dfrac{\xi}{3a_1}\Bigr),\
	\dfrac{\xi}{a_1}\Bigl(2-\dfrac{\xi}{a_1}\Bigr),\
	\dfrac{2}{a_1}\Bigl(1-\dfrac{\xi}{a_1}\Bigr)
	\right),
	& 0<\xi<a_1,\\[10pt]
	\left(\xi-\dfrac{a_1}{3},\ 1,\ 0\right),
	& a_1\le \xi<a_2,\\[10pt]
	\left(
	c-\displaystyle\int_\xi^1 t(s)\,\dd s,\
	\dfrac{(1+2\xi-3a_2)(1-\xi)^2}{(1-a_2)^3},\
	-\dfrac{6(1-\xi)(\xi-a_2)}{(1-a_2)^3}
	\right),
	& a_2\le \xi\le 1,\\[12pt]
	(c,\ 0,\ 0),
	& \xi>1.
...
\sigma_n(d_n)=
\begin{cases}
	\sigma_n^{\max}\,t(\bar d_n), & d_n\ge 0,\\[3pt]
	k_c\,d_n, & d_n<0,
\end{cases}
\qquad
k_c=\kappa\,\frac{\sigma_n^{\max}}{d_n^{\max}}.
\]
For the tangential mode we define the shear magnitude and the signed
traction as
\[
...
\begin{cases}
	\dfrac{\sigma_n^{\max}}{d_n^{\max}}\,g(\bar d_n), & d_n>0,\\[6pt]
	k_c, & d_n<0,
\end{cases}
\qquad
