...
circumferential (hoop) stress criterion and related energy-based arguments
\cite{ErdoganSih1963}.
Path conditions such as enforcing $K_{\rm II}=0$ at an advancing crack tip
\cite{CotterellRice1980} provide equivalent selection principles.
For cracks interacting with material interfaces, early analyses established
when a crack continues along the interface or kinks into an adjacent medium,
highlighting the dominant role of the mixed-mode elastic field in steering the
initial deflection \cite{HeHutchinson1989}.
...
crack.
Foundational models idealised the process zone using simple cohesive laws
\cite{Barenblatt1962,Dugdale1960}, and subsequent finite-element
...
\cite{CamachoOrtiz1996}.
Cohesive laws have also been central to interpreting and predicting resistance
curves \cite{TvergaardHutchinson1992} and to modelling dynamic crack growth
with intrinsic interface behaviour \cite{XuNeedleman1994}.

\paragraph{Mixed-mode fracture and cohesive laws.}
A comprehensive review by \chg{R1C7}{Park and Paulino} \cite{Park2011Review}
distinguishes two broad families of mixed-mode models:
\emph{energy-based} criteria and \emph{traction-based} cohesive laws.
Energy-based sche\-mes—such as the widely used Benzeggagh--Kenane (BK) criterion
\cite{BenzeggaghKenane1996}—prescribe a mode-dependent fracture toughness but
do not supply interface tractions and cannot be derived from a single
potential.
Traction-based cohesive laws, initiated by Xu and Needleman
\cite{XuNeedleman1994}, define an explicit mixed-mode potential
$\Psi(d_n,d_t)$ whose derivatives give the physical tractions.
Subsequent extensions introduced polynomial, power-law, exponential, and
asymmetric couplings, allowing the mode transition and shear sensitivity to be
tuned to the material system.
Such potential-based cohesive laws ensure thermodynamic consistency and yield
symmetric algorithmic tangents—properties advantageous for implicit finite
elements and essential for analysing mixed-mode initiation, kinking, and
propagation, which is the focus of the present work.

The importance of accurately describing the near-tip fracture process zone
(FPZ) has motivated several recent modelling developments.
Najmeddine, Gupta and Moini \cite{NajmeddineGuptaMoini2025} coupled a phase-field
description of bulk fracture with a PPR cohesive interface to capture
deflection, penetration and toughening mechanisms in multi-material systems,
while Shabrou and Daneshjoo \cite{ShabrouDaneshjoo2025} proposed mixed-mode
criteria for orthotropic materials that incorporate FPZ effects and T-stress,
improving directional predictions over classical LEFM criteria.
Analytical progress has likewise sharpened the understanding of mixed-mode
fields: Liu and Wei derived a closed-form stress field for finitely kinked
cracks \cite{Liu2021KinkedStressFields} and later a semi-analytical SIF
framework for branched cracks \cite{Liu2023BranchedSIF}, Si and Wei extended
these ideas to kinked edge cracks in a half-space \cite{Si2024KinkedEdge}, and
Shi and Xu obtained explicit SIFs for infinitesimal kinks in orthotropic
solids \cite{Shi2025OrthotropicKinkSIF}.
Parallel cohesive-zone studies have clarified how mode mixity, cohesive
strength and interface toughness compete to determine crack deflection versus
penetration \cite{ParmigianiThouless2006CZMDeflection,Alam2016IncidentAngleCZM,
	Tankasala2017KinkingOrthotropic,ChenDeBorst2022PFRCZM}.

\paragraph{Motivating example: why cohesive models remain essential.}
Even in configurations where LEFM is nominally applicable, cohesive-zone
effects can produce measurable changes in the predicted critical load.
For a mode-I crack of half-length $a$ in an infinite plate, LEFM predicts
...
singular elastic field by a finite process zone of length $\ell_{\mathrm{p}}$,
and consequently yields a slightly smaller critical load.

Even when the process-zone size is only about 5\% of the crack half-length
—a regime that is typically regarded as safely within the domain of LEFM—
the cohesive prediction remains 2–3\% lower than the LEFM
estimate.  This modest but systematic deviation demonstrates that the
presence of a finite process zone influences not just near-tip fields,
but also global quantities such as the critical load.

This example highlights a central theme of the present work:
while the \emph{kink direction} is governed almost exclusively by the
mixed-mode elastic field, the \emph{critical load} retains a clear
signature of the traction–separation law and of the finite FPZ.
...
critical load.
To this end, we employ a deliberately minimal plane-strain finite-element
formulation in which the physical crack is extended by a short,
zero-thickness cohesive continuation.
A slender auxiliary channel is introduced solely to control mesh resolution
and is collapsed prior to loading, ensuring that all tractions and boundary
conditions apply directly to the true crack faces.
For each trial orientation, we compute a \emph{critical state} dictated solely
by the prescribed traction--separation law and examine two complementary
selectors for the kink direction: a global load-based criterion and a local,
cohesive-energetic criterion at the mouth.
This philosophy avoids remeshing, enrichment, or geometry-specific heuristics
and reveals, with high transparency, which aspects of the cohesive law
influence the kink direction and which enter only through the evolution of the
fracture process zone and the critical load.


\paragraph{Structure of the paper.}
Section~\ref{sec:problem} introduces the problem statement, geometry and notation.
Section~\ref{sec:governing} presents the governing equations of the elastic bulk.
Section~\ref{sec:tsl} develops the cohesive interface formulation, mixed-mode potential,
and consistent linearisation.
Section~\ref{sec:construction} describes interface operators and assembly.
Section~\ref{sec:mesh} outlines mesh construction, channel collapse, and refinement.
Section~\ref{sec:results} reports numerical results, including field plots, convergence
studies, and kink selection.
Section~\ref{sec:LEFM-comparison} compares cohesive predictions with LEFM benchmarks.
Section~\ref{sec:conclusions} summarises the findings and future directions.
...
two-leg crack path that serves as the basis for studying mixed-mode crack kinking under steady-state conditions. The problem setup, geometry, local frames, and representative
traction–separation law are summarized in Fig.~\ref{fig:statement}.

The body, shown in Fig.~\ref{fig:stmt:a}, is a rectangular domain
subjected to uniform tensile tractions on its top and bottom edges, while the
vertical sides remain free of external loads. The internal discontinuity consists of two consecutive legs forming a polyline.
The first leg (Leg~1) represents the existing physical crack of a given length and orientation,
and the second leg (Leg~2) is a short fictitious extension used to realize a cohesive interface
where kinking may occur.
\chg{R2C1}{We denote by $\Gamma_{\mathrm{cr}}$ the boundary of the physical crack (Leg~1), and by
	$\Gamma_{\mathrm{cz}}$ the boundary of the cohesive extension (Leg~2).}
The kink originates at the common junction point of the two legs, here referred to as the \emph{knee},
and ends at the trial cohesive tip. The orientation of the second leg with respect to
the first defines the kink angle. The domain is homogeneous and isotropic,
and plane-strain conditions are assumed throughout.

A closer view of the two-leg configuration is presented in
Fig.~\ref{fig:stmt:b}. The physical crack, Leg~1, is traction-free,
whereas Leg~2 carries cohesive tractions acting on its two opposing faces.
These tractions depend on the local relative displacements between the upper
and lower surfaces, which are expressed in a local coordinate system rigidly attached
to Leg~2. The local frame comprises a tangential direction, aligned with the
cohesive segment, and a normal direction, pointing from the lower to the upper
face. The associated tractions form a pair of equal and opposite vectors
acting on the two sides of the cohesive zone.

The distribution of these tractions and the corresponding relative openings
are sketched in Fig.~\ref{fig:stmt:c}. Along the cohesive leg, the normal
opening and tangential sliding evolve according to the adopted
TSL. The region of active cohesive interaction is
bounded by the physical crack tip and the cohesive tip, while the rest of the
crack faces remain traction-free. The local coordinate axes $(s,r)$ are used
to describe variations of the opening and the traction along and across the
interface.

\paragraph{Choice of Cohesive Leg Length $\delta$.}
The cohesive leg length $\delta$ is an input parameter that defines the available extent for the process zone to develop. For the approach to regularize the singular elastic stress field and achieve a finite cohesive traction profile, $\delta$ must be sufficiently long to accommodate the full cohesive zone. Choosing $\delta$ too short artificially truncates the process zone, leaving a residual stress singularity at the cohesive tip—precisely the behavior that CZMs are designed to avoid. While larger $\delta$ values slightly increase computational cost, they do not affect the solution provided the active cohesive segment $\beta$ remains less than $\delta$. Accordingly, $\delta$ plays the role of a numerical regularization length rather than a
material parameter.

For trapezoidal cohesive laws, including the piecewise-smooth law used here, a practical estimate of the required $\delta$ can be obtained from the Dugdale model. In this idealization, the cohesive zone length $\ell_p$ scales with the crack size $a$ as
\[
\ell_p = 2 \pi a \left(\sigma_{\text{LEFM}}^{\text{cr}} / \sigma_{\max}\right)^2.
\]
For practical use, a value of $\delta = 2 \ell_p$ is generally sufficient to regularize the near-tip fields. This estimate applies generically to trapezoidal cohesive laws. For exponential or power-law potentials, similar estimates can be derived. The numerical examples in this work use $\delta/a = 0.06$, which falls well within this guideline and has been verified to yield fully developed, mesh-independent process zones.

\medskip

Figure~\ref{fig:stmt:d} shows the mode-wise traction–separation laws used in the
cohesive formulation.  In each pure mode, the traction increases from zero to the
cohesive strength and then decays smoothly to zero at complete decohesion.  The
initial rising segment is not intended to represent material hardening; instead, it
regularises the near-tip behaviour of the FPZ.  Without this strengthening
branch, imposing a constant plateau traction from $d_n=0$ would generate a jump in
stress at the point where the cohesive zone meets the traction-free crack faces,
leading to an unphysical stress singularity at the FPZ tip.  The rising
branch ensures that the traction smoothly approaches its maximum value, thereby
providing a finite-stress condition at the cohesive-zone tip and enabling a
geometrically smooth closure of the cohesive zone.  Because the extent of the FPZ is not known a priori, this regularisation also allows its length to emerge
naturally as part of the solution.


The main objective of the analysis is to determine the steady-state direction of crack
advance, or equivalently the kink angle, that develops in response to the applied mixed-mode
loading, by evaluating competing trial kink directions through the cohesive formulation.
 To this end, the subsequent sections formulate
the governing equations, define the cohesive law quantitatively, and describe
the finite-element implementation that enables numerical evaluation of the
critical state.
...
\subsection{Sensitivity to the auxiliary cohesive-leg length}

As discussed in Section~2, the prescribed length $\delta$ of the auxiliary cohesive continuation is a numerical parameter rather than a constitutive one. Its function is to provide sufficient space for the fracture-process zone to develop along the trial leg without artificial truncation. If $\delta$ is chosen too short, the active cohesive zone may approach the artificial tip of the segment, and the resulting critical load and selected kink direction may become dependent on the assumed trial-leg length. It is therefore important to verify that the reported results are not artifacts of an arbitrary choice of $\delta$.

To examine this point, we performed an additional series of calculations in which $\delta$ was not fixed a priori, but estimated adaptively from a Dugdale-type process-zone scaling. For each value of the material parameter ratio
\[
\eta=\frac{E}{\sigma_n^{\max}},
\]
and for each prescribed orientation $\theta_1$, the corresponding LEFM solution was first used to define an effective mixed-mode stress-intensity measure,
\[
K_{\mathrm{eff}}=\sqrt{K_I^2+K_{II}^2},
\]
from which a Dugdale-type estimate of the process-zone size was constructed as
\[
r_D=C_D\left(\frac{K_{\mathrm{eff}}}{\sigma_n^{\max}}\right)^2.
\]
The trial cohesive-leg length was then chosen as
\[
\delta=\frac{r_D}{r_\beta},
\]
where $r_\beta$ is the target value of $\beta/\delta$, with $\beta$ denoting the activated portion of the cohesive segment. In the present calculations, $r_\beta\approx 0.9$, so that the active zone remains sufficiently far from the segment tip while still being well resolved.

This procedure gives the algorithm a predictor--corrector structure. The LEFM field provides a first estimate of the characteristic process-zone size through $r_D$, whereas the subsequent cohesive calculation checks a posteriori whether the activated portion of the trial segment occupies the intended fraction of the cohesive leg. Table~\ref{tab:eta_sweep_all} summarizes the resulting values of the normalized trial-leg length $\delta/a$, the active-zone fraction $\beta/\delta$, and the selected angle $\theta_2^\ast$ for three representative initial crack inclinations.

\begin{table}[t]
	\centering
	\caption{Results of the $\eta$-sweep with adaptive estimation of $\delta$ for different initial angles $\theta_1$.}
	\label{tab:eta_sweep_all}
...
Three observations follow directly from Table~\ref{tab:eta_sweep_all}. First, the normalized trial length $\delta/a$ increases monotonically with $\eta$ for all three initial crack orientations. This trend is consistent with the Dugdale-type scaling $\delta\propto r_D\propto K_{\mathrm{eff}}^2/(\sigma_n^{\max})^2$: since $E$ is fixed and $\sigma_n^{\max}=E/\eta$, increasing $\eta$ reduces the cohesive strength and enlarges the expected process-zone size. The adaptive procedure therefore assigns a longer auxiliary cohesive leg as $\eta$ increases.

Second, although the absolute value of $\delta/a$ changes substantially across the sweep, the ratio $\beta/\delta$ remains nearly constant. For $\theta_1=-30^\circ$, it is exactly $0.9000$ throughout. For $\theta_1=-40^\circ$, it lies in the narrow interval $0.9125$--$0.9375$, and for $\theta_1=-50^\circ$ it also remains within $0.9125$--$0.9375$. This stability shows that the LEFM-based estimate of $\delta$ regularizes the finite cohesive problem in a quantitatively controlled manner: the active cohesive zone remains well contained within the trial segment over the entire investigated parameter range.

Third, the selected angle $\theta_2^\ast$ exhibits only weak sensitivity to the change in $\delta$ induced by the Dugdale-based estimate. Over the full range $\eta=80$--$120$, the total variation of $\theta_2^\ast$ is about $0.053^\circ$ for $\theta_1=-30^\circ$, about $0.083^\circ$ for $\theta_1=-40^\circ$, and about $0.298^\circ$ for $\theta_1=-50^\circ$. Thus, within the tested range, varying $\eta$ primarily rescales the process-zone size, whereas the predicted propagation direction remains governed mainly by the local mixed-mode state.

This point is important for interpreting the role of the cohesive parameters. The purpose of the $\eta$-sweep is not to claim that the direction is strictly independent of the cohesive law. Rather, it shows that, once the process zone is resolved in a consistent way through adaptive selection of $\delta$, the directional prediction is comparatively insensitive to moderate changes in $\eta$, while the finite cohesive length scale itself changes substantially. In this sense, the additional Dugdale-based calculations do not remove the intrinsic limitation associated with the auxiliary cohesive-leg length, but they do quantify and control it.

To assess whether the trends in Table~\ref{tab:eta_sweep_all} are artifacts of the baseline discretization, the calculations were repeated on a denser mesh. The refined computations leave the main conclusions unchanged. The values of $\delta/a$ remain unchanged to the reported precision, and $\beta/\delta$ is unchanged except for one isolated shift of $0.0125$ at $\theta_1=-50^\circ$, $\eta=115$. The largest absolute changes in the predicted angle $\theta_2^\ast$ remain small: they do not exceed $0.0064^\circ$ for $\theta_1=-30^\circ$, $0.0069^\circ$ for $\theta_1=-40^\circ$, and $0.0311^\circ$ for $\theta_1=-50^\circ$. These differences are an order of magnitude smaller than the total variation of $\theta_2^\ast$ across the full $\eta$-range and do not alter any of the trends discussed above. The refined-mesh recalculation therefore supports the numerical robustness of the reported conclusions.

\subsection{Limitations and range of applicability}

The present formulation is intended as an incipient-kinking model in a near-LEFM regime, that is, for situations in which the fracture-process zone remains small compared with the characteristic geometric dimensions of the body and is fully contained within the prescribed cohesive continuation. In this setting, the method provides a regularized and energetically consistent way to determine the critical load and compare competing kink directions without introducing singular crack-tip fields.

Accordingly, the proposed approach should not be interpreted as a general crack-propagation framework. The trial cohesive leg is introduced as an auxiliary numerical construct for direction selection, not as a physically evolved crack segment, and its length must be chosen sufficiently large so that the active cohesive zone does not reach the artificial tip. If this condition is violated, the process zone is truncated and both the critical load and the selected direction may become dependent on the chosen trial length. The method is therefore applicable only when the consistency condition $\beta < \delta$ is satisfied with adequate margin.

A second limitation concerns the constitutive structure of the cohesive law. The present mixed-mode law is potential-based and was adopted because it guarantees energetic consistency and a symmetric algorithmic tangent, which are advantageous for robust Newton iterations. However, this same energetic coupling implies that the traction components are not degraded independently. As a result, small residual normal tractions may persist when the tangential mode is dominant, and vice versa. This feature is well known for potential-based mixed-mode cohesive laws and should be kept in mind when interpreting local traction fields, especially at strong mode mixity.

The present study is also restricted to quasi-static loading, homogeneous isotropic elasticity, and a prescribed two-leg geometry. The conclusions regarding the close agreement between CZM and LEFM for the kink direction should therefore be understood within this scope. In particular, the recalculated parameter sweeps show that, over the investigated range, increasing the ratio $E/\sigma_n^{\max}$ mainly enlarges the cohesive-zone scale, while the predicted kink angle changes only weakly. This supports the conclusion that the deflection direction remains primarily controlled by the surrounding elastic mixed-mode field, whereas the critical load is more sensitive to the finite cohesive-zone structure. Outside this regime, for example when the process zone is not small compared with the crack length or nearby geometric features, larger deviations from LEFM should be expected.

Finally, the method does not by itself resolve crack growth beyond onset. Extending the formulation to incremental propagation would require repeated updating of the crack path and local remeshing around the advancing front. The present results should therefore be viewed as validating a robust initiation-and-kinking procedure rather than as a complete simulation framework for arbitrary crack evolution.
