function out = main_sweep_theta2(Pphys, varargin)
...
[cache, iL] = eval_trial_cached(cache, thetaL0_deg, Pphys, C0, dLeg, [], solverDisplay, S.useWarmStart, S.verbose);
[cache, iC] = eval_trial_cached(cache, thetaC0_deg, Pphys, C0, dLeg, [], solverDisplay, S.useWarmStart, S.verbose);
...
if S.verbose
    ...
    fprintf('theta* = %8.4f deg | ell_adv = %.6g | sig = %.6g | dt_mouth = %.3e\n', ...
        theta_star_deg, ell_star, sig_star, dt_star);
end
...
if verbose
    if trial.valid
        fprintf('  eval theta = %8.4f deg | flag = %d | sig = %.6g | dt = % .3e | ell = %.6g\n', ...
            theta_deg, trial.flag, trial.sig_cr, trial.dt_mouth, trial.ell_adv);
    else
