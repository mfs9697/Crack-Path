function h = fig_crack_trajectory(Path, varargin)
p = inputParser;
...
h = figure('Color','w'); clf; hold on; axis equal; box on;

% Outer domain / hole from config if available
if ~isempty(S.C0)
    A = S.C0.A; B = S.C0.B;
    ...
    if S.showHole && isfield(S.C0,'holes') && ~isempty(S.C0.holes)
        ...
        for i = 1:numel(holeLoops)
            H = holeLoops{i};
            patch(H(:,1), H(:,2), [1 1 1], 'EdgeColor','k', 'LineWidth',1.2);
        end
    end
end
...
title('Crack trajectory in the perforated domain', 'Interpreter','latex');
grid on;
end